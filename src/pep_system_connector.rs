...
pub fn create_pepsystem(resource_file: &str) -> PEPSystem {
    ...
    let pep_system_config: PEPSystemConfig  = serde_yml::from_str(&file_content)
        .expect("Failed to parse token file");

    let blinding_factor = BlindingFactor(ScalarNonZero::decode_from_hex(pep_system_config.blinding_factor.as_str()).expect("Failed to decode blinding factor"));
    
    PEPSystem::new(PseudonymizationSecret(pep_system_config.pseudonymization_secret), EncryptionSecret(pep_system_config.rekeying_secret), blinding_factor)
